...
from stretch.dynav.memory_map_localizer import EnhancedVoxelMapLocalizer
...
class ImageProcessor:
    def __init__(
        ...
        self.memory_file = memory_file or os.path.join(self.log, "object_memory.pkl")
    ...
    def create_vision_model(self):
        ...
        self.voxel_map_localizer = EnhancedVoxelMapLocalizer(
            self.voxel_map,
    ...
    def update_memory_after_navigation(self, object_category: str, location: np.ndarray, success: bool):
        with self.voxel_map_lock:
            if success:
                # Increase confidence for this location
                self.voxel_map_localizer.instance_memory.reinforce_location(
                    object_category, 
                    location, 
                    time.time()
                )
            else:
                # Decrease confidence if object wasn't found
                self.voxel_map_localizer.instance_memory.decrease_confidence(
                    object_category, 
                    location, 
                    time.time()
                )
    ...
    #     memory_data = {
    #         'instances': self.voxel_map_localizer.instance_memory.instances,
    #         'location_memory': self.voxel_map_localizer.instance_memory.location_memory,
    #         'timestamp': time.time()
    #     }
    ...
    #         self.voxel_map_localizer.instance_memory.instances = memory_data['instances']
    #         self.voxel_map_localizer.instance_memory.location_memory = memory_data['location_memory']
            
    #         # Apply time decay to all memories since last save
    #         current_time = time.time()
    #         last_time = memory_data['timestamp']
    #         self.voxel_map_localizer.instance_memory.apply_time_decay(current_time - last_time)
    ...
    def process_text(self, text, start_pose):
        ...
        if self.rerun:
            if self.rerun_visualizer is None:
                ...
                rr.log("world/memory_locations", rr.Clear(recursive=True))
            else:
                ...
                self.rerun_visualizer.clear_identity("world/memory_locations")
        ...
        # Check previous trajectory target if exists
        ...
        if waypoints is None:
            ...
            if text is not None and text != "" and localized_point is None:
                ...
                debug_text += debug_text_vg
                
                if localized_point is not None:
                    print("Target point selected through visual grounding!")
                    if self.rerun and self.rerun_visualizer is None:
                        rr.log(
                            "world/object",
                            rr.Points3D(
                ...
                # If visual grounding failed, try memory-guided search
                if localized_point is None and text is not None and text != "":
                    with self.voxel_map_lock:
                        memory_locations = self.voxel_map_localizer.instance_memory.get_search_locations(text, time.time())
                        
                    if memory_locations:
                        debug_text += f"## Visual grounding failed. Found {len(memory_locations)} remembered locations for {text}.\n"
                        # Visualize memory locations if rerun is enabled
                        if self.rerun:
                            for i, loc in enumerate(memory_locations):
                                color = torch.Tensor([0.5, 0.5, 1.0])  # Light blue for memory locations
                                if self.rerun_visualizer is None:
                                    rr.log(
                                        "world/memory_locations",
                                        rr.Points3D(
                                            [loc[0], loc[1], 1.5],
                                            colors=color,
                                            radii=0.1,
                        ...
                        # Try each memory location until one works
                        for loc in memory_locations:
                            if self.voxel_map_localizer.verify_point(text, loc):
                                localized_point = loc
                                mode = "memory_search"
                                debug_text += "## Successfully verified remembered location.\n"
                                print("Successfully verified remembered location.")
                                break
            ...
            # If both visual grounding and memory search fail, do frontier exploration
            if text is None or text == "" or localized_point is None:
                debug_text += "## Both visual grounding and memory search failed, starting frontier-based exploration.\n"
        ...
        if waypoints is not None:
            ...
            if finished:
                self.traj = None
                # Update memory after successful navigation
                if mode == "memory_search":
                    with self.voxel_map_lock:
                        self.voxel_map_localizer.instance_memory.update_instance(
                            text,
                            0,  # instance_id doesn't matter for reinforcement
                            localized_point,
                            1.0,  # high confidence since we found it
                            time.time()
                        )
            else:
    ...
    def process_rgbd_images(self, rgb, depth, intrinsics, pose):
        ...
        """Process RGB-D images and update both spatial and semantic memory."""
        ...
        with self.voxel_map_lock:
            ...
            # Process image with SAM if we're actively searching for an object
            if hasattr(self, 'active_search_object'):
                masks, scores = self.voxel_map_localizer.process_image_with_sam(
                    rgb.permute(1, 2, 0).cpu().numpy()
                )
                
                # Update instance memory for each detected instance
                for mask, score in zip(masks, scores):
                    if score > self.voxel_map_localizer.instance_memory.confidence_threshold:
                        instance_depth = depth[mask]
                        if len(instance_depth) > 0:
                            # Calculate 3D centroid of the instance
                            instance_xyz = world_xyz[mask]
                            centroid = instance_xyz.mean(dim=0)
                            
                            # Update instance memory
                            self.voxel_map_localizer.instance_memory.update_instance(
                                self.active_search_object,
                                self.obs_count,  # Use observation count as instance ID
                                centroid.cpu().numpy(),
                                score,
                                time.time()
                            )

        # Run mask CLIP and add to voxel PCD
